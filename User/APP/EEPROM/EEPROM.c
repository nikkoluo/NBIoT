...
u8 EEPROM_Read_Data(u16 usRead_Addr, u8* pBuffer, u16 *pLen);           // 读数据
...
* Return         :  1成功 0失败
*******************************************************************************/
u8 EEPROM_Read_Data(u16 usRead_Addr, u8* pBuffer, u16 *pLen)
{
	u16 usADD = 0;
	u8 Buffer[2];
	u16 i;
	...
	// 长度
	*pLen = (Buffer[0] << 8) + Buffer[1];

	// 读取数据和累加
	EEPROM_Read_Buffer(usRead_Addr, pBuffer, (*pLen) + 2);
	...
	if (usADD == ((pBuffer[(*pLen) - 2] << 8) + pBuffer[(*pLen) - 1])) 
	{
		Result = 1;
	}
...
}// End of u8 EEPROM_Read_Data(u16 usRead_Addr, u8* pBuffer, u16 *pLen)
