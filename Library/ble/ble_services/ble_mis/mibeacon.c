 ...
 * LOCAL VARIABLES
 */
static uint16_t mibeacon_frameCtrl; //@RETENTION MEMORY;

static uint8_t  mibeacon_capability; //@RETENTION MEMORY;;
static uint8_t  mibeacon_mac[B_ADDR_LEN]; //@RETENTION MEMORY;;
static uint8_t  mibeacon_evt[MIBEACON_MAX_EVENT_LEN]; //@RETENTION MEMORY;;
static uint8_t  mibeacon_evtLen ; //@RETENTION MEMORY;;
static uint8_t  mibeacon_title[MIBEACON_MAX_EVENT_LEN]; //@RETENTION MEMORY;;
static uint8_t  mibeacon_titleLen ; //@RETENTION MEMORY;;
static uint8_t  mibeacon_mfData[MIBEACON_MAX_EVENT_LEN] ; //@RETENTION MEMORY;;
...
mi_sts_t mibeacon_append(uint8_t* advData, uint8_t len, uint8_t* totalLen)
{
    ...
    *totalLen = mibeacon_len + len;
    ...
    //*p++ = mibeacon_len - 1;
    //*p++ = ADV_TYPE_SERVCIE_DATA;

    //*p++ = LO_UINT16(MISERVICE_UUID);
    //*p++ = HI_UINT16(MISERVICE_UUID);
